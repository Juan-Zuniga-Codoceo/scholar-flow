...
def test_billing_flow():
    print("🚀 Iniciando pruebas del sistema de suscripción SaaS con Mercado Pago...")
    ...
    # 4. Probar POST /billing/pay (generación de preferencia MP)
    ...
    print(f"👉 Enlace de Pago Mercado Pago Generado: {pay_data['payment_url']}")
    ...
    mp_preference_id = db_payment["flow_token"]
    flow_order = db_payment["flow_order"]
    print(f"👉 MP Preference ID (flow_token) en DB: {mp_preference_id}")
    print(f"👉 MP Order (flow_order) en DB: {flow_order}")

    # 5. Simular llamada al Webhook POST /billing/webhook con el formato de notificación de Mercado Pago
    print("🔹 Simulando llamada del Webhook de Mercado Pago...")
    
    # Mockear get_payment_status de MercadoPagoService
    from mercado_pago_service import MercadoPagoService
    original_get_status = MercadoPagoService.get_payment_status
    
    # Reemplazar temporalmente la función para retornar éxito
    ...
    # Enviar webhook de Mercado Pago
    webhook_payload = {
        "type": "payment",
        "data": {
    ...
    print("🎉 ¡TODAS LAS PRUEBAS DE MERCADO PAGO PASARON EXITOSAMENTE! 🎉")
